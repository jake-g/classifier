...
Status PrintTopLabels(const std::vector<Tensor>& outputs,
  ...
  const int how_many_labels = std::min(25, static_cast<int>(label_count));
  Tensor indices;
  Tensor scores;
  ...
  for (int pos = 0; pos < how_many_labels; ++pos) {
    ...
    std::cout << "\"" << pos << "\": " << "{" ;
    std::cout << "\"breed\": \"" << labels[label_index] << "\",";
    std::cout << "\"score\": \"" << score << "\"}, ";
  }
  std::cout << "\"" << how_many_labels << "\": " << "{" ;
  std::cout << "\"breed\": \"other\",";
  std::cout << "\"score\": \"" << (1 - score_sum) << "\"}";
  std::cout << "}\n";
  return Status::OK();
}
